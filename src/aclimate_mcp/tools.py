...
from aclimatesdkpy.aclimate_models import PointDataRequest, PointDataResponse

# Lazy provider: resolves the shared client inside the running event loop.
GetClient = Callable[[], Awaitable[AClimateClient]]


def register_tools(mcp, get_client: GetClient) -> None:
    # ═══════════════════════════════════════════════════════════════════════════
    ...
    async def search_locations_by_name(name: str) -> list[object]:
        ...
        client = await get_client()
        data = await client.get_locations_by_search(q=name)
    ...
    @mcp.tool(name="get_point_data_from_coordinates",
            description="Get a raster time series for a geographic point (latitude/longitude) from a GeoServer workspace and store. Dates must be ISO format (YYYY-MM-DD).")
